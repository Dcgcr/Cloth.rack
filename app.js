...
function makeCard(imgUrl, caption, meta = {}){
  ...
  // clicking Grab This will add the item to the cart and open WhatsApp with a prefilled message (including the image URL)
  grabBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    const imageEl = wrap.querySelector('img');
    if (!imageEl || !imageEl.src) return;

    // add to local cart (keeps current behavior)
    addToCart({ image: imageEl.src, caption: caption || '' });
    showCardToast(card, 'Added to buy list');

    // Build WhatsApp prefill message including the image URL so it is forwarded as a link
    const waBase = 'https://wa.me/2349066477623';
    const text = `Don Courage, I just checked out this outfit from your website and I would love to negotiate with you. Image: ${imageEl.src}`;
    const waUrl = `${waBase}?text=${encodeURIComponent(text)}`;

    // Open WhatsApp chat in new tab/window (WhatsApp will show the link preview if available)
